...
import {columns, scanCodeFields} from './stock-count-recount-detail.config.js';
...
import Styles from './StockCountRecountDetail.style';
import {notificationService} from '../../../../services/notificationService';

class StockCountRecountDetail extends Component {
  constructor(props) {
    super(props);
    this.state = {
      pageSize: 10,
  ...
  getPartsOfRecount = async () => {
    const recountDetail = await this.getStockCountRecount();
    if (recountDetail) {
      let parts = get(recountDetail, 'NvStkHdToItm', []);
      if (!isArray(parts)) {
        parts = values(parts);
      }
      this.setState({
        list: parts,
        Hh_Doc: recountDetail.Hh_Doc,
        recountDetail,
        fromUnfinish: recountDetail.Hh_UnfinishedDoc,
      });
    }
  };

  getStockCountRecount = () => {
  ...
  saveToSAP = async () => {
    // Get State
    let {list, Hh_Doc, recountDetail: recount} = this.state;
    ...
    if (recount) {
      ...
      let itemClone = await deepCopy(list),
        headerClone = await deepCopy(recount);
      // Case: non login
      if (!token) {
        // TODO: save offline
        ...
        stockCountService.saveStockCountRecount(headerClone).then(res => {
  ...
  goToLogin = id => {
    ...
    navigate('Login', {
      data: {
        headerId: id,
        detailPage: 'StockCountRecountDetail',
      },
    });
  };

  excDeleteOffline = () => {
    const {fromUnfinish, recountDetail} = this.state;
    if (!fromUnfinish) {
      this.getPartsOfRecount();
    } else {
      // Delete local
      deleteRow(tableConstant.name.DOC_HEADER, recountDetail.id).then(() => {
        this.goToList();
      });
    }
  };
  ...
  saveOffline = async (notify, isTimeout) => {
    let {list, Hh_Doc, recountDetail} = this.state;
    // Clone data avoid out scope transaction from list
    let itemClone = await deepCopy(list),
      headerClone = await deepCopy(recountDetail);
    update(tableConstant.name.DOC_HEADER, Hh_Doc, itemClone, headerClone)
      .then(() => {
        if (isTimeout) {
          return;
  ...
  goToList = () => {
    ...
    const {navigate} = this.props.navigation;
    navigate(
      !fromUnfinish ? 'StockCountRecountList' : 'UnfinishedDocumentList',
    );
  };
  ...
  callMessage = message => {
    ...
    switch (message) {
      case MESSAGE.M001:
        messObj = {
          content: t(MESSAGE.M001, {fieldName: t('Re-Counted-Msg')}),
          buttons: [
            {
          ...
          <Button
            ...
            <Text>
              ...
              <Text style={Styles.textColor}>
                : {formatHhDoc(recountDetail.Hh_Doc)}
              </Text>
            </Text>
            <Icon
              type="FontAwesome"
        ...
        {isExpanded && (
          <View style={[isPortTrait ? Styles.viewDataDetails : '']}>
            ...
            <Text style={[isPortTrait ? Styles.flexWidth : Styles.autoWidth]}>
              ...
              {moment(recountDetail.Sap_DocDate).format('DD.MM.YYYY')}
            </Text>
            <Text style={[isPortTrait ? Styles.flexWidth : Styles.autoWidth]}>
              <Text style={Styles.labelBold}>{t('Random Count No.')}: </Text>
              {recountDetail.Sap_Doc}
            </Text>
            <Text style={[isPortTrait ? Styles.flexWidth : Styles.autoWidth]}>
              <Text style={Styles.labelBold}>{t('Year')}: </Text>
              {recountDetail.Sap_DocYear}
            </Text>
          </View>
        )}
        <View
...
export default withTranslation()(StockCountRecountDetail);
