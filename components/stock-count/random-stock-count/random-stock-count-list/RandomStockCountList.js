...
} from './random-stock-count-list.config';
...
import {randomStockStyle} from './RandomStockCountList.style';

class RandomStockCountList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onSearch = () => {
    ...
    filterList(tableConstant.trx_type.RANDOM_COUNT, filterData).then(data => {
  ...
  deleteDBBeforeInsert = async listItem => {
    ...
    deleteTrx_Type(
      ...
      tableConstant.masterKey.C_RANDOM,
      tableConstant.trx_type.RANDOM_COUNT,
    )
      .then(() => {
        insert(tableConstant.name.MASTER, {
          id: tableConstant.masterKey.C_RANDOM,
          Trx_Type: tableConstant.trx_type.RANDOM_COUNT,
        ...
        <Row size={80}>
          <View style={viewEditable}>
            <EditableTable
              ...
              columns={columns}
              values={list}
              detailPath={'RandomStockCountDetail'}
...
export default withTranslation()(RandomStockCountList);
