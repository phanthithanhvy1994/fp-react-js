...
import {columns, scanCodeFields} from './random-stock-count-detail.config';
...
import Styles from './RandomStockCountDetail.style';
...
class RandomStockCountDetail extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getPartsOfRandom = async () => {
    const randomDetail = await this.getRandomStock();
    if (randomDetail) {
      let parts = get(randomDetail, 'NvStkHdToItm', []);
      ...
      this.setState({
        list: parts,
        Hh_Doc: randomDetail.Hh_Doc,
        randomDetail,
        fromUnfinish: randomDetail.Hh_UnfinishedDoc,
      });
    }
  };
  ...
  saveToSAP = async () => {
    let {list, Hh_Doc, randomDetail} = this.state;
    ...
    if (randomDetail) {
      ...
      let itemClone = await deepCopy(list),
        headerClone = await deepCopy(randomDetail);
      // Case: non login
      if (!token) {
        // TODO: save offline
        ...
        stockCountService.saveRandomStock(headerClone).then(res => {
  ...
  goToLogin = id => {
    ...
    navigate('Login', {
      data: {
        headerId: id,
        detailPage: 'RandomStockCountDetail',
      },
    });
  };
  ...
  excDeleteOffline = () => {
    // Get State
    const {fromUnfinish, randomDetail} = this.state;
    if (!fromUnfinish) {
      this.getPartsOfRandom();
    } else {
      deleteRow(tableConstant.name.DOC_HEADER, randomDetail.id).then(() => {
        this.goToList();
      });
    }
  };
  ...
  goToList = () => {
    ...
    const {navigate} = this.props.navigation;
    navigate(!fromUnfinish ? 'RandomStockCountList' : 'UnfinishedDocumentList');
  };
          ...
          <Button
            ...
            <Text>
              ...
              <Text style={Styles.textColor}>
                : {formatHhDoc(randomDetail.Hh_Doc)}
              </Text>
            </Text>
            <Icon
              type="FontAwesome"
        ...
        {isExpanded && (
          <View style={[isPortTrait ? Styles.viewDataDetails : '']}>
            ...
            <Text style={[isPortTrait ? Styles.flexWidth : Styles.autoWidth]}>
              ...
              {moment(randomDetail.Sap_DocDate).format('DD.MM.YYYY')}
            </Text>
            <Text style={[isPortTrait ? Styles.flexWidth : Styles.autoWidth]}>
              <Text style={Styles.labelBold}>{t('Random Count No.')}: </Text>
              {randomDetail.Sap_Doc}
            </Text>
            <Text style={[isPortTrait ? Styles.flexWidth : Styles.autoWidth]}>
              <Text style={Styles.labelBold}>{t('Year')}: </Text>
              {randomDetail.Sap_DocYear}
            </Text>
          </View>
        )}
        <View
...
export default withTranslation()(RandomStockCountDetail);
