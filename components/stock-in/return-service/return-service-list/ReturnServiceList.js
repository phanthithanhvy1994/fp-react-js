...
} from './return-service-list.config';
...
import {returnServiceStyle} from './ReturnServiceList.style';

class ReturnServiceList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onSearch = () => {
    ...
    filterList(tableConstant.trx_type.SALES_RETURN, filterData).then(data => {
  ...
  deleteDBBeforeInsert = async listItem => {
    ...
    deleteTrx_Type(
      ...
      tableConstant.masterKey.I_SALESRTN,
      tableConstant.trx_type.SALES_RETURN,
    )
      .then(() => {
        insert(tableConstant.name.MASTER, {
          id: tableConstant.masterKey.I_SALESRTN,
          Trx_Type: tableConstant.trx_type.SALES_RETURN,
        ...
        <Row size={80}>
          <View style={viewEditable}>
            <EditableTable
              ...
              columns={columns}
              values={list}
              detailPath={'ReturnServiceDetail'}
...
export default withTranslation()(ReturnServiceList);
