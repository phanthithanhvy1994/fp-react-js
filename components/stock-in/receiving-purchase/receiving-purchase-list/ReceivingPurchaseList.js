...
} from './receiving-purchase-list.config';
...
import {receivingPurchaseStyle} from './ReceivingPurchaseList.style';

class ReceivingPurchaseList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onValueChange = e => {
    ...
    // when input quick search => filterField is initial value
  ...
  onSearch = () => {
    ...
    filterList(tableConstant.trx_type.RECEIVE_PURCHASE, filterData).then(
      data => {
  ...
  deleteDBBeforeInsert = async listItem => {
    ...
    deleteTrx_Type(
      ...
      tableConstant.masterKey.I_PURCHASE,
      tableConstant.trx_type.RECEIVE_PURCHASE,
    )
      .then(() => {
        insert(tableConstant.name.MASTER, {
          id: tableConstant.masterKey.I_PURCHASE,
          Trx_Type: tableConstant.trx_type.RECEIVE_PURCHASE,
        ...
        <Row style={isPortTrait ? rowContainer : flexContainer}>
          <Col style={[viewSearch, isPortTrait ? '' : viewMargin]}>
            <View style={isPortTrait ? flexRow : viewBtn}>
              <Row style={[rowSearch, isPortTrait ? rowPadding : '']} s>
                <Field
        ...
        <Row size={80}>
          <View style={viewEditable}>
            <EditableTable
              ...
              columns={columns}
              values={list}
              detailPath={'ReceiveFromPurchasingDetail'}
              loadingMore={loadingMore}
              onSubmitEditing={this.onSubmitEditing}
              customStyles={!isPortTrait ? tableStyles : flexTable}
...
export default withTranslation()(ReceivingPurchaseList);
