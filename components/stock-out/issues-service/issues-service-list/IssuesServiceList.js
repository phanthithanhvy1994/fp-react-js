...
} from './issues-service-list.config';
...
import {issuesServiceStyle} from './IssuesServiceList.style';

class IssuesServiceList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  deleteDBBeforeInsert = async listItem => {
    ...
    deleteTrx_Type(
      ...
      tableConstant.masterKey.O_SALES,
      tableConstant.trx_type.SALES_OUT,
    ).then(() => {
      insert(tableConstant.name.MASTER, {
        id: tableConstant.masterKey.O_SALES,
        Trx_Type: tableConstant.trx_type.SALES_OUT,
  ...
  onSearch = () => {
    ...
    filterList(tableConstant.trx_type.SALES_OUT, filterData).then(data => {
  ...
  onFilter = async filterData => {
    ...
    if (this._isMounted) {
      stockOutService
        .filterIssuesService(filterData)
        .then(res => {
          const {results} = res.d;
          if (results) {
        ...
        <Row size={80}>
          <View style={viewEditable}>
            <EditableTable
              ...
              columns={columns}
              values={list}
              detailPath={'IssuesServiceDetail'}
...
export default withTranslation()(IssuesServiceList);
